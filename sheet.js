...
// Function to highlight selected rows and operation columns
function highlightSelection() {
    const rowFrom = parseInt(document.getElementById('row-range-from').value, 10) || 1;
    const rowTo = parseInt(document.getElementById('row-range-to').value, 10) || rowFrom;
    const operationColumns = document.getElementById('operation-columns').value.trim().toUpperCase().split(',');
    ...
    rows.forEach(row => {
        ...
        row.querySelectorAll('td').forEach(cell => cell.style.backgroundColor = ''); // Reset cell background
    });

    // Apply new highlights for the specified rows and operation columns
    rows.forEach((row, rowIndex) => {
        // Highlight only if the row index is within the specified range
        if (rowIndex >= rowFrom - 1 && rowIndex <= rowTo - 1) {
            operationColumns.forEach(col => {
                // Convert column letter(s) to index
                const colIndex = col.charCodeAt(0) - 65; // A = 0, B = 1, etc.
                // Highlight the specified cell in the row if it exists
                if (!isNaN(colIndex) && colIndex >= 0 && row.cells[colIndex]) {
                    row.cells[colIndex].style.backgroundColor = '#fff9c4'; // Highlight specified cell
                }
            });
        }
    });
...
// Event listeners to call highlightSelection when the user updates the row and column range
document.getElementById('row-range-from').addEventListener('input', highlightSelection);
document.getElementById('row-range-to').addEventListener('input', highlightSelection);
document.getElementById('operation-columns').addEventListener('input', highlightSelection);
